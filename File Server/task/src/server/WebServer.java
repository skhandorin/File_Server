...
public class WebServer {
    ...
    public void start() {
        ...
        try {
            // Закрываться serverSocket будет в треде-обработчике клиентского запроса при получении EXIT
            serverSocket = new ServerSocket(port, 50, InetAddress.getByName(address));

            while (true) {
                // Закрываться socket будет в треде-обработчике клиентского запроса после завершения обработки
                Socket socket = serverSocket.accept();
                executor.submit(() -> handleClientRequest(socket));
            }
        } catch (SocketException e) {
            // При остановке вебсервера ожидание очередного обращения к серверу serverSocket.accept() в main-thread
            // всегда завершается по эксепшену SocketException - иначе никак...
    ...
    private void handleClientRequest(Socket socket) {
        ...
        try {
            ...
            if (received.equals(EXIT_REQ)) {
                // Говорим серверу, что надо завершать работу прямо сейчас
                stop();
            }
